...
const FolderCard = ({folder, onClick, onShare, onRename, onMove, onDelete}) => {
  return (
      ...
      rounded-2xl p-4 transition-all duration-200 cursor-pointer flex items-center justify-between'>
          <button type='button' onClick={()=>onClick(folder)} className=' flex-1 flex items-center gap-3 min-w-0 pr-2 text-left'>
          ...
          <ItemDropdown item={folder} onShare={onShare} onRename={onRename} onMove={onMove} onDelete={onDelete}></ItemDropdown>
      </div>
  )
}
